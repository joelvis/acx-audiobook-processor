...
def process_audio_file(input_path, output_path):
    ...
    try:
        ...
        ffmpeg_command = [
            ...
            'acompressor=threshold=-18dB:ratio=2:attack=20:release=1000,'  # Compression
            'volume=volume=2.1,'  # Increase volume by 2.1 dB
            'loudnorm=I=-20:TP=-3:LRA=11:measured_TP=1,'  # RMS normalization to -20dB with strict peak limiting
            'alimiter=level_in=0.7:level_out=0.7:limit=0.5:attack=3:release=100',  # Stricter limiter settings
            # Output format settings
            '-ar', '44100',      # 44.1kHz sample rate
            '-ac', '1',          # Mono output
            '-codec:a', 'libmp3lame',
